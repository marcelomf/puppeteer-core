...
import { DeviceRequestPrompt, DeviceRequestPromptDevice, DeviceRequestPromptManager, } from './DeviceRequestPrompt.js';
...
describe('DeviceRequestPrompt', function () {
    describe('waitForDevicePrompt', function () {
        ...
        it('should respect timeout', async () => {
            ...
            const timeoutSettings = new TimeoutSettings();
            const manager = new DeviceRequestPromptManager(client, timeoutSettings);
            await expect(manager.waitForDevicePrompt({ timeout: 1 })).rejects.toBeInstanceOf(TimeoutError);
        });
        it('should respect default timeout when there is no custom timeout', async () => {
            ...
            await expect(manager.waitForDevicePrompt()).rejects.toBeInstanceOf(TimeoutError);
        });
    ...
    describe('DeviceRequestPrompt.waitForDevice', function () {
        ...
        it('should respect timeout', async () => {
            ...
            await expect(prompt.waitForDevice(({ name }) => {
                return name.includes('Device');
            }, { timeout: 1 })).rejects.toBeInstanceOf(TimeoutError);
        });
        it('should respect default timeout when there is no custom timeout', async () => {
            const client = new MockCDPSession();
            const timeoutSettings = new TimeoutSettings();
    ...
    describe('DeviceRequestPrompt.select', function () {
        ...
        it('should error for device not listed in devices', async () => {
            ...
            await expect(prompt.select(new DeviceRequestPromptDevice('11111111', 'Device 1'))).rejects.toThrowError('Cannot select unknown device!');
        });
        it('should fail when selecting prompt twice', async () => {
            ...
            await prompt.select(device);
            await expect(prompt.select(device)).rejects.toThrowError('Cannot select DeviceRequestPrompt which is already handled!');
        });
    });
    describe('DeviceRequestPrompt.cancel', function () {
        ...
        it('should fail when canceling prompt twice', async () => {
            ...
            await prompt.cancel();
            await expect(prompt.cancel()).rejects.toThrowError('Cannot cancel DeviceRequestPrompt which is already handled!');
        });
    });
});
